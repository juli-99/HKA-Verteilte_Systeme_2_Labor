...
from constMutex import ENTER, RELEASE, ALLOW, DELETE, ALIVE
...
class Process:
    ...
    def __allowed_to_enter(self):
        ...
        #  print(f'{self.process_id}: lm {processes_with_later_message} : op {self.other_processes}')
        return first_in_queue and all_have_answered
    
    def __delete_nodes(self, crashed_nodes: list) -> None:
        """ delete the given nodes and their messages """
        self.logger.info(f'{self.__mapid()} is deleting {crashed_nodes}')
        for n in crashed_nodes: # remove crashed nodes
            if n in self.other_processes:
                self.other_processes.remove(n)
            if n in self.all_processes:
                pass#self.all_processes.remove(n)
        for req in self.queue:
            if req[1] in crashed_nodes:
                self.queue.remove(req)  # remove requests from crashed node
        self.__cleanup_queue()  # cleanup the queue

    def __confirm_crash(self, node: str) -> bool:
        self.channel.send_to([node], (self.clock, self.process_id, ALIVE))
        rep = self.channel.receive_from([node], 2)
        return not rep
    
    def __check_for_and_remove_crashed_nodes(self) -> None:
        """ tries to detect crashed nodes, deletes them and shares them with the network """
        possible_crashes: list = []
        crashed_nodes: list = []
        for n in self.other_processes:
            if n not in list([req[1] for req in self.queue[1:]]):
                possible_crashes.append(n)
        if len(possible_crashes) > 0: 
            for n in possible_crashes:
                if self.__confirm_crash(n):
                    self.logger.info(f'{self.__mapid()} has confirmed that {self.__mapid(n)} has crashed.')
                    crashed_nodes.append(n)
            self.__delete_nodes(crashed_nodes)  # delete nodes and their messages
            self.channel.send_to(self.other_processes, (self.clock, self.process_id, DELETE, crashed_nodes))
    ...
    def __receive(self):
        ...
        if _receive:
            ...
            if msg[2] == ENTER:
                ...
                self.__delete_nodes(msg[3])
            elif msg[2] == ALIVE:
                self.channel.send_to([_receive[0]], (self.clock, self.process_id, ALIVE))

            self.__cleanup_queue()  # Finally sort and cleanup the queue
        else:        
            self.logger.warning("{} timed out on RECEIVE.".format(self.__mapid()))
            self.__check_for_and_remove_crashed_nodes()
