...
from constMutex import ENTER, RELEASE, ALLOW, HB
...
class Process:
    ...
    def __allowed_to_enter(self):
        ignored_processes: list = [p for p in self.other_processes if time.time() - self.hb[p] > 15]
        #if len(ignored_processes) > 0: 
            #print(f'{self.__mapid()} is ignoring {[self.__mapid(p) for p in ignored_processes]}')
        self.queue = [rep for rep in self.queue if rep[1] not in ignored_processes]
        ...
        all_have_answered = len(self.other_processes) == len(processes_with_later_message) + len(ignored_processes)
        return first_in_queue and all_have_answered
    
    def __receive(self):
        if time.time() - self.last_hb > 1:
            self.send_hb()
        # Pick up any message
        ...
        if _receive:
            ...
            if msg[2] == ENTER:
                ...
                self.hb.update({msg[1]: time.time()})
    ...
    def send_hb(self) -> None:
        """ send heartbeat to other processes """
        self.channel.send_to(self.other_processes, (self.clock, self.process_id, HB))
        self.last_hb = time.time()
