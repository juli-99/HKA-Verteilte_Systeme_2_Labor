        ...
        while repeat:
            if _receive:
                ...
                if msg[2] == ENTER:
                    ...
                    self.channel_heartbeat[msg[1]] = time.time()
                    if not self.all_processes.__contains__(msg[1]):
                        self.all_processes.append(msg[1])
                    if self.process_id != msg[1] and not self.other_processes.__contains__(msg[1]):
                        self.other_processes.append(msg[1])
                    #print(self.channel_heartbeat)
                    _receive = self.channel.receive_from(self.other_processes, 10)
                elif msg[2] == RELEASE:
            ...
            #for key, val in self.channel_heartbeat.items():
            #    if current_time - val > 6:
                    #print(key)
                    #print(self.all_processes)
                    #print("\n\n\n\n")
            #        if self.all_processes.__contains__(key):
            #            self.all_processes.remove(key)
            #        if self.other_processes.__contains__(key):
            #            self.other_processes.remove(key)
        ...
        while True:
            ...
            for key, val in self.channel_heartbeat.items():
                if cur_time - val > 8:
                    ...
                    if self.other_processes.__contains__(key):
                        self.other_processes.remove(key)
                    if key != self.process_id and self.all_processes.__contains__(key):
                        self.all_processes.remove(key)
                    drop_keys.append(key)
            for key in drop_keys:
                self.channel_heartbeat.pop(key)
            print(self.all_processes)
            time.sleep(5)
