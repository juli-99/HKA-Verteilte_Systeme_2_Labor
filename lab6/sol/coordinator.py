...
class Coordinator:
    ...
    def run(self):
        ...
        # received VOTE_COMMIT from all participants
        ...
        # Collect answers from all participants
        yet_to_receive = list(self.participants)
        timeouts = 0
        while len(yet_to_receive) - timeouts > 0:
            msg = self.channel.receive_from(self.participants, TIMEOUT)
            if (not msg):
                timeouts += 1
                print('timeout while waiting for READY_COMMIT')
            else:
