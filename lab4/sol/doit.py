...
class DummyChordClient:
    ...
    def run(self):
        node: str = str(choice(list(self.channel.channel.smembers('node'))).decode())  # get random node
        key: int = int(random() * int(max(list(self.channel.channel.smembers('node')))))  # get random key
        print(f'Starting LOOCKUP {'{:04n}'.format(key)} at {'{:04n}'.format(int(node))}')
        self.channel.send_to({node}, (constChord.LOOKUP_REQ, key, str(self.node_id)))  # send lookup key to node
        rep = self.channel.receive_from_any()  # rcv reply fom final node
        print(f'LOOCKUP {'{:04n}'.format(key)} complete: Succ({'{:04n}'.format(key)}) =' +
                f' {'{:04n}'.format(rep[1][1])}')  # print reply
