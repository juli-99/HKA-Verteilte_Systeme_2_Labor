...
class ChordNode:
    ...
    def remote_successor_node(self, key: int, origin: str) -> None | int:
        """
        recursive LookUp
        :param key: key to be located
        :param origin: client that initiated lookup
        :return: located node name
        """
        best_node = self.local_successor_node(key)  # get best local node
        if best_node == self.node_id:
            return self.node_id  # if current node is succ(key) -> return it
        else:
            self.channel.send_to({str(best_node)}, (constChord.LOOKUP_REQ, key, origin))  # send lookup to next node
            return None  # this node is not the final node
    ...
    def run(self):
        while True:  # Start node operation loop
            ...
            if request[0] == constChord.LOOKUP_REQ:  # A lookup request
                ...
                next_id: None | int = self.remote_successor_node(request[1], request[2])
                if next_id:
                    self.channel.send_to([request[2]], (constChord.LOOKUP_REP, next_id))
                    
                # Finally do a sanity check
                if next_id and not self.channel.exists(next_id):  # probe for existence
